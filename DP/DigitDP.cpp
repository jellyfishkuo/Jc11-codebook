用途： 求解閉區間 $[L, R]$ 中滿足特定條件（如：不包含特定數位、特定數位出現次數、特殊整除性質）的數字個數。
// 範例：求在 [L, R] 之間有多少個數字不包含數位 '4'
int digits[20];
long long dp[20][2]; // dp[當前位數][是否受到上限限制]

long long dfs(int pos, bool limit, bool lead) {
    if (pos < 0) return 1; // 成功構造一個合法數字
